...
def load_and_preprocess_data() -> pd.DataFrame:
    ...
    easter_saturday_data = pd.read_csv(os.path.join(PROJECT_ROOT, '0_DataPreparation/input/compiled_data/easter_saturday.csv'),
                                     skiprows=1, names=['date', 'is_easter_saturday'])
    easter_saturday_data['date'] = pd.to_datetime(easter_saturday_data['date'])
    ...
    # Add position in month (Anfang/Mitte/Ende)
    temporal_features['position_in_month'] = temporal_features['day'].apply(
        lambda x: 'Anfang' if x <= 10 else ('Mitte' if x <= 20 else 'Ende')
    )
    position_dummies = pd.get_dummies(temporal_features['position_in_month'], prefix='month_position')
    temporal_features = pd.concat([temporal_features, position_dummies], axis=1)
    
    # Add 1 day before holiday feature
    holidays_dates = set(holidays_data['date'].dt.date)
    def is_day_before_holiday(date):
        next_day = date.date() + pd.Timedelta(days=1)
        return 1 if next_day in holidays_dates else 0
    temporal_features['is_day_before_holiday'] = temporal_features['date'].apply(is_day_before_holiday)
    ...
    # Add temperature categories (base and season-adjusted)
    def get_base_temp_category(temp):
        if temp < 5:
            return 'kalt'
        elif temp < 15:
            return 'mild'
    ...
    def get_seasonal_temp_category(row):
        temp = row['Temperatur']
        month = row['month']
        
        # Winter (Dec, Jan, Feb)
        if month in [12, 1, 2]:
            return ('kalt' if temp < 0 else
                   'mild' if temp < 8 else
                   'warm')
        # Spring/Fall (Mar, Apr, May, Sep, Oct, Nov)
        elif month in [3, 4, 5, 9, 10, 11]:
            return ('kalt' if temp < 8 else
                   'mild' if temp < 18 else
                   'warm')
        # Summer (Jun, Jul, Aug)
        else:
            return ('kalt' if temp < 15 else
                   'mild' if temp < 25 else
                   'warm')
    
    print("Adding temperature categories...")
    # Create base temperature categories
    weather_features['temp_category_base'] = weather_features['Temperatur'].apply(get_base_temp_category)
    temp_base_dummies = pd.get_dummies(weather_features['temp_category_base'], prefix='temp_base')
    weather_features = pd.concat([weather_features, temp_base_dummies], axis=1)
    ...
    weather_features['temp_category_seasonal'] = weather_features.apply(get_seasonal_temp_category, axis=1)
    temp_seasonal_dummies = pd.get_dummies(weather_features['temp_category_seasonal'], prefix='temp_seasonal')
    weather_features = pd.concat([weather_features, temp_seasonal_dummies], axis=1)
    weather_features.drop(['month'], axis=1, inplace=True)
    ...
    # Add Easter Saturday features
    final_df = pd.merge(final_df, easter_saturday_data,  
                       left_on='date', right_on='date', how='left')
    ...
    print("Handling missing values and string columns...")
    # Drop string/object columns that are not needed for feature selection
    columns_to_drop = ['date', 'Datum', 'position_in_month', 'temp_category_base', 'temp_category_seasonal']
    ...
    # Ensure Easter Saturday is properly filled
    final_df['is_easter_saturday'] = final_df['is_easter_saturday'].fillna(0).astype(int)
...
def main() -> None:
    try:
        ...
        features_path = os.path.join(output_dir, 'selected_features_v4.txt')
        with open(features_path, 'w') as f:
            f.write('\n'.join(selected_features))
        print(f"Saved selected features to: {features_path}")

        # Save all features with scores to file
        all_features_path = os.path.join(output_dir, 'all_features_importance_v4.txt')
        ...
        # Save plot
        plot_path = os.path.join(output_dir, 'feature_importance_v4.png')
