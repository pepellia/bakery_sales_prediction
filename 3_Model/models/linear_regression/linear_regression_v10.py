#!/usr/bin/env python3
# linear_regression_v10.py
"""
Linear Regression Model (v10) - Combining best practices from v8 and v9:
...
def load_and_prepare_data():
    ...
    # Might need to parse 'id' -> date & Warengruppe
...
def create_pipeline(weather_condition_cols):
    """Create the pipeline with structured numeric/categorical/binary features."""
    # 1. We define subsets of features (similar to v8)
    numeric_features = [
        "Wochentag",
        "week_sin", "week_cos",
        "month_sin","month_cos",
    ...
    categorical_features = ["Warengruppe"]  # We'll one-hot encode Warengruppe
    ...
    # Potentially add selective weather conditions
    ...
    valid_weather_cols = [c for c in weather_condition_cols if c in selected_weather_conditions]
    ...
    # We'll define a final feature_columns list for convenience
    # (so we know what to pass for test predictions)
...
def main():
    # 1) Load & prepare data
    logging.info("Loading and preparing data (v10)...")
    ...
    # 2) Identify columns in the weather dummies
    weather_condition_cols = [c for c in train_df.columns if c.startswith("weather_")]
    
    # 3) Build and train pipeline
    logging.info("\nTraining model...")
    model, feature_columns = create_pipeline(weather_condition_cols)
...
def generate_submission_predictions(model, feature_columns, test_df):
    ...
    out_path = os.path.join(OUTPUT_DIR, "submission_v10.csv")
