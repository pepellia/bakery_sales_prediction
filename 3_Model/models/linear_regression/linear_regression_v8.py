...
def load_and_prepare_data():
    ...
    for df in [train_data, val_data, test_df]:
        ...
        df = pd.merge(df, easter_saturday_data, on='Datum', how='left')
        df['is_easter_saturday'] = df['is_easter_saturday'].fillna(0).astype(int)
        ...
        df = pd.merge(df, public_holidays_data[['Datum', 'Feiertag']], on='Datum', how='left')
        df['is_public_holiday'] = df['Feiertag'].fillna(0).astype(int)
        df.drop('Feiertag', axis=1, inplace=True)
        ...
        df = pd.merge(df, windjammer_data[['Datum', 'Windjammerparade']], on='Datum', how='left')
        df['is_windjammer'] = df['Windjammerparade'].fillna(0).astype(int)
        df.drop('Windjammerparade', axis=1, inplace=True)
...
def generate_submission_predictions(model, feature_columns, train_df):
    ...
    pred_data['is_public_holiday'] = pred_data['Feiertag'].fillna(0).astype(int)
    pred_data.drop('Feiertag', axis=1, inplace=True)
    
    # Load Windjammer data
    windjammer_data = pd.read_csv(WINDJAMMER_PATH)
    windjammer_data['Datum'] = pd.to_datetime(windjammer_data['Datum'])
    pred_data = pd.merge(pred_data, windjammer_data[['Datum', 'Windjammerparade']], on='Datum', how='left')
    pred_data['is_windjammer'] = pred_data['Windjammerparade'].fillna(0).astype(int)
    pred_data.drop('Windjammerparade', axis=1, inplace=True)
