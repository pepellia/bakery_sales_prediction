#!/usr/bin/env python3
# linear_regression_v12.py
"""
Linear Regression Model (v12) - Based on v11, adding adjusted R² metric:
 ...
 - Added adjusted R² metric for better model evaluation
"""
...
def adjusted_r2_score(y_true, y_pred, n_features):
    ...
    n_features : int
        Number of features (predictors) in the model
...
def save_metrics(train_metrics, val_metrics, feature_importance):
    """Save metrics and model equation to a text file."""
    current_time = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
    
    with open(METRICS_FILE, 'w') as f:
        f.write(f"{current_time} - Model Metrics\n")
        f.write("-" * 40 + "\n\n")
        ...
        f.write(f"  Adjusted R²: {train_metrics['adj_r2']:.3f}\n\n")
        
        # Validation metrics
        f.write("Validation Metrics:\n")
        f.write(f"  RMSE: {val_metrics['rmse']:.2f}\n")
        f.write(f"  MAE:  {val_metrics['mae']:.2f}\n")
        f.write(f"  R²:   {val_metrics['r2']:.3f}\n")
        f.write(f"  Adjusted R²: {val_metrics['adj_r2']:.3f}\n\n")
        
        # Model equation
        f.write("Model Equation:\n")
        f.write("Sales = β₀ + \n")
        f.write("        # Very High Importance Features\n")
        f.write("        β₁(is_silvester) + β₂(is_summer) + β₃(temp_base_warm) + \n")
        f.write("        β₄(is_weekend) + β₅(is_good_weather) + β₆(product_group) +\n")
        f.write("        Σᵢ βᵢ(school_holiday_stateᵢ) +\n\n")
        
        f.write("        # High Importance Features\n")
        f.write("        β₇(day_of_week) + β₈(temperature) + β₉(feels_like_temp) +\n")
        f.write("        β₁₀(cloud_cover) + β₁₁(year) + β₁₂(quarter) +\n\n")
        
        f.write("        # Moderate Importance Features\n")
        f.write("        β₁₃(is_winter) + β₁₄(temp_base_cold) + β₁₅(temp_base_mild) +\n")
        f.write("        β₁₆(is_day_before_holiday) + β₁₇(is_easter_saturday) +\n")
        f.write("        β₁₈(is_month_end) + β₁₉(is_kieler_woche) +\n\n")
        
        f.write("        # Lower Importance Features\n")
        f.write("        β₂₀(is_spring) + β₂₁(is_fall) + β₂₂(is_public_holiday) +\n")
        f.write("        β₂₃(is_windjammer) + Σᵢ βᵢ(weather_conditionᵢ)")

def evaluate_model(y_true, y_pred, n_features, dataset_name=""):
    """Compute RMSE, MAE, R^2 and adjusted R² and print them."""
    ...
    logging.info(f"  Adjusted R²: {adj_r2:.3f}")
...
def main():
    # 1) Load & prepare data
    logging.info("Loading and preparing data (v12)...")
    ...
    train_metrics = evaluate_model(train_df['Umsatz'], train_pred, len(feature_columns), "Training")
    val_metrics = evaluate_model(val_df['Umsatz'], val_pred, len(feature_columns), "Validation")
    
    # Save metrics to file
    feature_importance = None  # TODO: Add feature importance if needed
    save_metrics(train_metrics, val_metrics, feature_importance)
